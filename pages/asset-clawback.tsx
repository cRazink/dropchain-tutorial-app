...
import { ClawbackATokenNftCode } from '@/devlink/ClawbackATokenNftCode';
import React, { useState } from 'react';

export default function ClawbackATokenNftCodePage() {
  const [outputText, setOutputText] = useState('');

  const sendRequest = async () => {
    setOutputText('DropChain API request sent, processing...');

    const url = '/v1/clawback_asset_testnet'; 
    const options = {
      method: 'POST',
      headers: {
